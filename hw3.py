...
KEYWORDS = []   # a list of words to be read from a file
...
def directory_files(directory_path):
    filepaths = []
...
def full_feature_vectors(negative_files, positive_files, tfidf_transform=True):
    """
    ...
    :param tfidf_transform: If this parameter is true, the feature vectors aren't returned as raw counts,
     but normalized using tf-idf representation, using scikit's TfidfTransformer
    ...
    if tfidf_transform:
        all_vectors = TfidfTransformer().fit_transform(all_vectors)
...
def select_k_best(negative_files, positive_files, k=50):
    ...
    :return: (neg_feature_vecs, pos_feature_vecs, features), where the first two are two dimensional arrays of size
     ...
     The neg_feature_vecs are raw counts of the number of occurrences of each word in each file
    """
...
def best_features_vectors(negative_files, positive_files, k=50, tfidf_transform=True):
    """
    Calculates the k best features, using scikit's SelectKBest, and then calculates the feature vectors, using
     CountVectorizer, and optionally also applies tfidf_transform
    The difference between this function and select_k_best is that this one uses CountVectorizer, and also allows
     TfIdf transformation before returning the vectors.

    :param negative_files: A list of negative review files
    :param positive_files: A list of positive review files
    :param tfidf_transform: If this parameter is true, the feature vectors aren't returned as raw counts,
     but normalized using tf-idf representation, using scikit's TfidfTransformer
    :return: Returns three values: neg_feature_vecs, pos_feature_vecs, feature_names
    """
    all_files = negative_files + positive_files

    _, _, feature_names = select_k_best(negative_files, positive_files, k=k)
    counter = CountVectorizer(input='filename', vocabulary=feature_names)
    ...
    negative_count = len(negative_files)
    return all_vectors[:negative_count], all_vectors[negative_count:], feature_names
...
class Review:
    ...
    def get_manual_feature_vector(self):
        ...
        if KEYWORDS is None or len(KEYWORDS) == 0:
            print('Cannot create manual feature vector before the KEYWORDS global variable is set.')
            return None
...
def estimate_accuracy(classifier, negative_samples, positive_samples, cross_val_folds=10):
    ...
    tenfold_validation = cross_validation.KFold(arr_length(all_samples), n_folds=cross_val_folds, shuffle=True)
...
def save_words(filename, words):
    text = reduce(lambda old, word: old + word + '\n', words, '')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        ...
        print('Usage: python <>.py <input_dir> <words_file_input_path> <best_words_file_output_path>')
        exit(1)
    input_folder = sys.argv[1]
    keywords_path = sys.argv[2]
    best_words_output_path = sys.argv[3]

    KEYWORDS = load_words(keywords_path)

    # -------- Classifying by manual vectors ----------

    print('step1 (manually generated features):', end='\n\n')
    ...
    neg_full_vectors, pos_full_vectors, feature_names = full_feature_vectors(negative_files, positive_files)
    ...
    print('step4 (selected best features):', end='\n\n')

    neg_reviews_best_features, pos_reviews_best_features, feature_names = \
        best_features_vectors(negative_files, positive_files)

    save_words(best_words_output_path, feature_names)
