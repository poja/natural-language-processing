...
def str_ngram(ngram_w_value, amplify=1):
    ...
    return '{: <35}\t{:2f}'.format(ngram, value * amplify)
...
if __name__ == '__main__':
    ...
    corpus = directory_file_contents(input_folder)
    # save_to_file(corpus, output_folder + '/full_corpus.txt')
    corpus = corpus.split('\n')
    for i in range(len(corpus)):
        corpus[i] = list(filter(lambda x: x != '', corpus[i].split(' ')))

    # Collocation Raw Frequency
    collocation_rawfreq_dict = collocation_raw_frequencies(corpus, to_tokenize=False)
    ...
    # Collocation PMI
    collocation_pmi_dict = collocation_pmi_values(corpus, to_tokenize=False, wordcount_filter=WORDCOUNT_FILTER)
    ...
    # Trigram PMI A
    trigram_pmi_dict = trigram_pmi_values(corpus, 'a', to_tokenize=False, wordcount_filter=WORDCOUNT_FILTER)
    trigram_pmi = best_ngrams(trigram_pmi_dict)
    str_output = reduce(lambda prev, cur: prev + str_ngram(cur) + '\n', trigram_pmi, '')
    save_to_file(str_output, output_folder + '/' + OUTPUT_FILE_PMI_TRI_A)

    # Trigram PMI B
    trigram_pmi_dict = trigram_pmi_values(corpus, 'b', to_tokenize=False, wordcount_filter=WORDCOUNT_FILTER)
    trigram_pmi = best_ngrams(trigram_pmi_dict)
    str_output = reduce(lambda prev, cur: prev + str_ngram(cur) + '\n', trigram_pmi, '')
    save_to_file(str_output, output_folder + '/' + OUTPUT_FILE_PMI_TRI_B)

    # Trigram PMI C
    trigram_pmi_dict = trigram_pmi_values(corpus, 'c', to_tokenize=False, wordcount_filter=WORDCOUNT_FILTER)
    trigram_pmi = best_ngrams(trigram_pmi_dict)
    str_output = reduce(lambda prev, cur: prev + str_ngram(cur) + '\n', trigram_pmi, '')
